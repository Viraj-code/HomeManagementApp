      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center space-x-4">
          <Button 
            ...
            className="relative"
            onClick={() => {
              const urgentCount = todaysActivities.filter(activity => 
                new Date(activity.startTime).getTime() - new Date().getTime() < 60 * 60 * 1000 && 
                !activity.completed
              ).length;
              
              const incompleteShoppingItems = totalShoppingItems - (shoppingLists?.reduce((acc, list) => 
                acc + list.items.filter(item => item.completed).length, 0) || 0);
              
              toast({
                title: "Notifications",
                description: `${urgentCount} upcoming activities, ${incompleteShoppingItems} shopping items pending`,
              });
            }}
          >
            <Bell className="w-4 h-4" />
            {(todaysActivities.filter(activity => 
              new Date(activity.startTime).getTime() - new Date().getTime() < 60 * 60 * 1000 && 
              !activity.completed
            ).length > 0) && (
