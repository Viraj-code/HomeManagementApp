...
export function MealPlanner() {
  ...
  const handleAddMeal = async (suggestion: MealSuggestion, mealType: string) => {
    try {
      ...
      const mealData = {
        name: suggestion.name || "Untitled Meal",
        description: suggestion.description || "",
        cuisine: suggestion.cuisine || "International",
        ingredients: suggestion.ingredients || [],
        instructions: suggestion.instructions || "",
        prepTimeMinutes: suggestion.prepTimeMinutes || 30,
        servings: suggestion.servings || 4,
        mealType
      };
      
      const createdMeal: any = await createMealMutation.mutateAsync({ meal: mealData, mealType });
      
      // If we have a selected date, also create a meal plan
      if (selectedDate && createdMeal?.id) {
